...
func (caa Counter) IsValid(s compandauth.SessionCAA, delta int64) bool {
	sessionCAA := abs(int64(s))
	// fmt.Println(sessionCAA)
	delta = abs(delta)
...
func (caa *Counter) Issue() compandauth.SessionCAA {
	sessionCAA := compandauth.SessionCAA(caa.abs())
	caa.step(1)
...
// var _ = compandauth.CAA(NewCounter())
